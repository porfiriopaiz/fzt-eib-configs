...
cp "$PWD/root-config/etc/systemd/system/my-first-run.service" "$ROOT_PATH/etc/systemd/system/"

# Copy the bash script.
cp "$PWD/root-config/usr/local/bin/my-first-run.sh" "$ROOT_PATH/usr/local/bin/"
...
ln -s /etc/systemd/system/my-first-run.service "$ROOT_PATH/etc/systemd/system/multi-user.target.wants/"
